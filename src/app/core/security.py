...
async def _get_active_user_by_identity(username_or_email: str, db: AsyncSession) -> dict[str, Any] | None:
    if "@" in username_or_email:
        return await crud_users.get(db=db, email=username_or_email, is_deleted=False)

    return await crud_users.get(db=db, username=username_or_email, is_deleted=False)
...
async def authenticate_user(
    ...
    db_user = await _get_active_user_by_identity(username_or_email=username_or_email, db=db)
    if not db_user:
        return False
