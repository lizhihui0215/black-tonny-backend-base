...
alembic_db_target = resolve_alembic_target(os.getenv("ALEMBIC_DB_TARGET"), config.config_file_name)

if alembic_db_target == "capture":
    config.set_main_option("sqlalchemy.url", settings.CAPTURE_DB_URL)
else:
    config.set_main_option("sqlalchemy.url", settings.SERVING_DB_URL)

if config.config_file_name is not None:
    fileConfig(config.config_file_name)
target_metadata: MetaData = load_target_metadata(alembic_db_target)
version_table = get_version_table(alembic_db_target)
